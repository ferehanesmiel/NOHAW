...
const CourseDetailPage: React.FC = () => {
    ...
    const currentProgress = (course && progress[course.id]) || 0;

    const handleProgressUpdate = () => {
        if(!course) return;
        // Simple progress: for now, one click completes the course
                    ...
                    <div className="bg-white rounded-lg shadow-md p-8">
                        ...
                        <div className="space-y-4">
                            {course.content?.map(block => (
                                <div key={block.id} className="p-4 border-l-4 border-slate-200">
                                    {block.type === 'text' && <p className="text-slate-700">{block.value}</p>}
                                    {block.type === 'image' && <img src={block.value} alt="course content" className="rounded-md" />}
                                    {block.type === 'video' && (
                                        <div className="aspect-w-16 aspect-h-9">
                                            <iframe src={block.value} title="Course Video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full rounded-md"></iframe>
                                        </div>
                                    )}
                                </div>
                            ))}
